...
60+ skills para desenvolvimento fullstack com Claude Code. Suporta Fastify, Express, NestJS, AdonisJS, Hono, Elysia, Next.js, React, Vue e mais.
...
Ou instalacao rapida:

```bash
curl -fsSL https://raw.githubusercontent.com/jhollyfer/maiyu-skills/main/install.sh | bash
```

Ou uso local (sem instalar):

```bash
claude --plugin-dir /caminho/para/maiyu-skills-skills
```
...
Todas as skills seguem estas convencoes de codigo:

1. Zero `any` — usar tipos concretos, `unknown`, generics
2. Zero ternarios — usar if/else, early return, const mapper
3. Zero `as TYPE` (exceto `as const`) — usar type guards, generics
4. Retorno sempre tipado — funcoes e componentes com tipo de retorno explicito
5. Multiplas condicoes → const mapper (object lookup)
